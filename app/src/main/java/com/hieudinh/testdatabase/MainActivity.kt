...
class MainActivity : AppCompatActivity() {
    ...
    private  lateinit var edClass2 : EditText
    ...
    private fun updateStuden() {
        ...
        val classs2 = edClass2.text.toString()

        if(name == std?.name && email==std?.email && classs==std?.classs && classs2==std?.classs2){
            Toast.makeText(this,"don't have  changed ",Toast.LENGTH_SHORT).show()
            return
        }
        if(std == null)  return
          val     std = StudentModel(id =std!!.id,name= name ,email= email,classs=classs ,classs2=classs2)
    ...
    private  fun addStudent(){
        ...
        if(name.isEmpty() || email.isEmpty() || classs.isEmpty() || classs2.isEmpty()){
            Toast.makeText(this,"enter your information",Toast.LENGTH_SHORT).show()
        }else{
            val     std=StudentModel(name=name,email = email,classs = classs ,classs2 = classs2)
...
private  fun initRecycleView(){
    ...
    private  fun  initView(){
        ...
        edClass2=findViewById(R.id.edClasss2)
